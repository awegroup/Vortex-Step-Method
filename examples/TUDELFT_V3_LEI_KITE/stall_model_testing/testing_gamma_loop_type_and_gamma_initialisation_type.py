...
save_folder = (
    ...
    / "stall_model_testing"
    / "results"
    / "gamma_loop_type_and_gamma_initialisation_type"
)
...
plot_polars(
    ...
    label_list=[
        ...
        "linear_elliptical",
        "linear_cosine",
        "non_linear_Claude_zero",
        "non_linear_Claude_elliptical",
        "non_linear_Claude_cosine",
        "non_linear_ChatGPT_zero",
        "non_linear_ChatGPT_elliptical",
        "non_linear_ChatGPT_cosine",
    ],
...
alpha = [17, 18, 19, 20, 21, 22, 23, 24, 25]
for alpha in alpha_range:
    body_aero_polar.va_initialize(Umag, alpha, side_slip, yaw_rate)

    results_gamma_loop_zero = solver_gamma_loop_zero.solve(
        body_aero_polar, gamma_distribution=None
    )
    results_gamma_loop_elliptical = solver_gamma_loop_elliptical.solve(
        body_aero_polar, gamma_distribution=None
    )
    results_gamma_loop_cosine = solver_gamma_loop_cosine.solve(
        body_aero_polar, gamma_distribution=None
    )
    results_gamma_loop_non_linear_Claude_zero = (
        solver_gamma_loop_non_linear_Claude_zero.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    results_gamma_loop_non_linear_Claude_elliptical = (
        solver_gamma_loop_non_linear_Claude_elliptical.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    results_gamma_loop_non_linear_Claude_cosine = (
        solver_gamma_loop_non_linear_Claude_cosine.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    results_gamma_loop_non_linear_ChatGPT_zero = (
        solver_gamma_loop_non_linear_ChatGPT_zero.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    results_gamma_loop_non_linear_ChatGPT_elliptical = (
        solver_gamma_loop_non_linear_ChatGPT_elliptical.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    results_gamma_loop_non_linear_ChatGPT_cosine = (
        solver_gamma_loop_non_linear_ChatGPT_cosine.solve(
            body_aero_polar, gamma_distribution=None
        )
    )
    plot_distribution(
        ...
        results_list=[
            results_gamma_loop_zero,
            results_gamma_loop_elliptical,
            results_gamma_loop_cosine,
            results_gamma_loop_non_linear_Claude_zero,
            results_gamma_loop_non_linear_Claude_elliptical,
            results_gamma_loop_non_linear_Claude_cosine,
            results_gamma_loop_non_linear_ChatGPT_zero,
